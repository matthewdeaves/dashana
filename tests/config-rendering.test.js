...
const BACKUP_CSV = path.join(__dirname, "fixtures/config-test-backup.csv");
const BACKUP_CONFIG = path.join(
  __dirname,
  "fixtures/config-test-backup.config",
);
...
// Swap in disabled config, build, then restore
beforeAll(() => {
  ...
  fs.copyFileSync(DISABLED_CONFIG, PRODUCTION_CONFIG);

  // Build site with disabled config
