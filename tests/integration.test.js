...
describeIntegration('Production Data Integration Tests', () => {
  ...
  test('all metric sums equal total task count', () => {
    let sectionSum = 0;
    $dashboard('.metric-sections .section-count').each((i, el) => {
      sectionSum += parseInt($dashboard(el).text());
    });

    let statusSum = 0;
    $dashboard('.metric-status .status-count').each((i, el) => {
      statusSum += parseInt($dashboard(el).text());
    });

    let prioritySum = 0;
    $dashboard('.metric-priority .priority-count').each((i, el) => {
      prioritySum += parseInt($dashboard(el).text());
    });

    let assigneeSum = 0;
    $dashboard('.metric-assignees .assignee-count').each((i, el) => {
      assigneeSum += parseInt($dashboard(el).text());
    });
